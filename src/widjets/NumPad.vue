...
// Определяем props для получения начальных данных
const props = defineProps({
  initialValue: {
    type: Number,
    default: 0
  },
  // Поле нужно, чтобы знать, когда показывать переключатель %/₸
  field: {
    type: String,
    required: true
  },
  // Получаем начальный тип скидки
  initialDiscountType: {
    type: String,
    default: 'percent'
  }
...
// Определяем события для отправки данных родителю
const emit = defineEmits(['confirm', 'cancel']);

const displayValue = ref('');
const discountType = ref('percent'); // 'percent' или 'fixed'
...
onMounted(() => {
  ...
  console.log(`NumPad mounted with initial value: ${displayValue.value} and discount type: ${discountType.value}`);
});

// --- Функции для управления вводом ---
function append(char) {
  if (char === '.' && displayValue.value.includes('.')) return; // Нельзя добавить больше одной точки
...
function handleConfirm() {
  ...
  if (!isNaN(value)) {
    // Отправляем объект с новым значением и типом скидки (если это поле скидки)
    emit('confirm', {
      value: value,
      discountType: discountType.value
    });
  } else {
    emit('cancel'); // Если введено что-то некорректное, просто отменяем
  }
}
